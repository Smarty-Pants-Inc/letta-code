      ...
      if (subtype === "initialize") {
        ...
        try {
          ...
          for (let attempt = 0; attempt < 2; attempt++) {
            if (!blockId) {
              const conversation =
                await _client.conversations.retrieve(conversationId);
              ...
              const results = await Promise.allSettled(
                isolatedIds.map(async (id) => {
                  const blk = await _client.blocks.retrieve(id);
            ...
            try {
              await _client.blocks.update(blockId, { value });
              break;
            } catch (e) {
